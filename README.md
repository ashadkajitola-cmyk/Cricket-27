    ...
    <div id="dashboardSection" class="hidden">
        ...
        <div id="adminPanelSection" class="card hidden" style="border: 2px solid var(--accent); background: #fffde7;">
            ...
            <div id="mainAdminCodeBox" class="hidden" style="background: #fff9c4; padding: 10px; border-radius: 6px; margin-bottom: 10px;">
                ...
                <input type="number" id="adminCodePoints" placeholder="Kitنے points ka code hai?">
            ...
            <div style="background: #e0f2f1; padding: 10px; border-radius: 6px; margin-bottom: 15px; border: 1px solid var(--primary);">
                <h4 style="margin: 0 0 8px 0; color: var(--primary-dark);">🏆 Points Table Management</h4>
                <input type="text" id="ptTableNameInput" placeholder="Table Name (jaise: ICC World Test Championship)">
                <input type="text" id="ptTeamNameInput" placeholder="Team Name (jaise: India)">
                ...
                <button class="action-btn" id="savePointTableTeamBtn" style="margin-top: 10px; background: var(--primary-dark);">Team Points Table Mein Jodein</button>
...
<script>
    ...
    function checkAdminAccess() {
        ...
        let hasActiveSub = (userData.subscription && userData.subExpiry > Date.now()) || (userData.subscription && userData.subscription.includes('Match') && userData.matchesLeft > 0) || (currentPhone === ADMIN_NUMBER);
        
        if(!hasActiveSub) {
            alert("Please purchase this subscription aapke pass koi active subscription nahi he");
            switchTab('wallet');
            return;
        }
    ...
    // Points Table Admin Actions
    function savePointTableTeam() {
        let tableName = document.getElementById('ptTableNameInput').value.trim();
        let teamName = document.getElementById('ptTeamNameInput').value.trim();
        ...
        if(!tableName || !teamName) {
            alert("Table Name aur Team Name bharna anivarya hai!");
            return;
        }
        ...
        document.getElementById('ptTeamNameInput').value = "";
        ...
        renderAllPointsTables();
        renderAdminPointsTablesList();
        alert("Team points table mein jodh di gayi hai!");
    }
    ...
    function renderAdminPointsTablesList() {
        ...
        if(keys.length === 0) {
            container.innerHTML = "<p style='font-size:12px; color:#666;'>Abhi koi points table nahi bani hai.</p>";
            return;
        }
